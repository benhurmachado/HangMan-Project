from funcoes_defs import Clear, Linha, Title1, Title2, fimdeJogo, Historico, hasNumbers, forca
...
def palavraDef():
    ...
    while True:
        while hasNumbers(palavraChave) or len(palavraChave) == 0 or palavraChave in "!@#$%&*()_+=-[{]}´`¨~^;><.,/\|]'":
                print("\nA palavra chave não pode ser numero,caractere nem ser deixada em branco\n")
...
def dicasDef():
    ...
    for c in range(1, 4):
        ...
        while True:
            if len(dica) == 0:
                print('\n==A dica não pode ficar em branco!==')
                dica = input(str(f'Digite a {c}ª dica: '))
            else: 
                dicas.append(dica)
                break
...
def dicasList():
    ...
    while True:
        if dicasTotais == 3:
            print(f'A dica 1 é: {dicas[0]}\n')
            del(dicas[0])
            break

        elif dicasTotais == 2:
            print(f'A dica 2 é: {dicas[0]}\n')
            del(dicas[0])
            break

        elif dicasTotais == 1:
            print(f'A dica 3 é: {dicas[0]}\n')
            del(dicas[0])
            break
